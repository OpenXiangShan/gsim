...
class UInt {
 public:
  ...
  UInt<width> operator/(UInt<w>& right) {
    if (width <= 64)
      return UInt<width>(data[0] / right.data[0]);
    else
      TODO();
    return right;
  }
  ...
  UInt<1> operator!=(UInt<width>& right) { return UInt<1>(1) - (*this == right); }
  ...
  UInt<1> operator>(UInt<width>& right) {
    ...
    while (leftIdx >= 0) {
      if (data[leftIdx] > right.data[rightIdx])
        return UInt<1>(true);
      else if (data[leftIdx--] < right.data[rightIdx--])
        return UInt<1>(false);
    }
    return UInt<1>(false);
  }
  ...
  //   return data[0] != 0;
  // }
  operator uint64_t() { return data[0]; }
};
