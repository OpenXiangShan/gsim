...
EXECUTABLE_GRH_VEC_REG_WRITES_TEST = executable-grh-vector-register-writes
EXECUTABLE_GRH_VEC_REG_WRITES_CHECK = $(FIR_TEST_INPUT_DIR)/check-$(EXECUTABLE_GRH_VEC_REG_WRITES_TEST).py
EXECUTABLE_GRH_VEC_REG_WRITES_TARGET = $(FIR_TEST_OUTPUT_DIR)/$(EXECUTABLE_GRH_VEC_REG_WRITES_TEST)/.done
...
$(EXECUTABLE_GRH_VEC_REG_WRITES_TARGET): $(FIR_TEST_INPUT_DIR)/$(EXECUTABLE_GRH_VEC_REG_WRITES_TEST).fir \
		$(EXECUTABLE_GRH_VEC_REG_WRITES_CHECK) $(GSIM_BIN)
	@mkdir -p $(@D)
	set -o pipefail && $(TIME) $(FIR_TEST_TIMEOUT_PREFIX) $(GSIM_BIN) $(GSIM_FLAGS_EXTRA) \
		--flatten-nodes --export-executable-grh=$(@D)/model.json --stop-after-stage=PreCoarsen \
		--dir=$(@D) $< 2>&1 | tee $(@D)/gsim.log
	python3 $(EXECUTABLE_GRH_VEC_REG_WRITES_CHECK) $(@D)/model.json
	@touch $@
...
test-executable-grh-vector-register-writes: $(EXECUTABLE_GRH_VEC_REG_WRITES_TARGET)
...
.PHONY: run-fir-test fir-tests test-executable-grh-split-register-clock \
	...
	test-executable-grh-vector-register-writes
