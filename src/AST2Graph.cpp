...
public:
  ...
  std::string abspath(const char *sep, std::string name) {
    return (path.empty() ? "" : cwd() + sep) + name;
  }
  void cd(const char *sep, std::string str) {
    path.push(abspath(sep, str));
  }
...
TypeInfo* Context::visitFields(PNode* fields, NodeType parentType) {
  ...
  for (int i = 0; i < fields->getChildNum(); i ++) {
    ...
    if (!fieldInfo->isAggr()) { // The type of field is ground
      Node* fieldNode = allocNode(curType, path->abspath(SEP_AGGR, field->name), fields->lineno);
...
*/
void Context::visitRegDef(PNode* reg, PNodeType type) {
...
Node* Context::add_member(Node* parent, std::string name, int idx, int width, int sign) {
  Node* member = allocNode(NODE_MEM_MEMBER, path->abspath(SEP_AGGR, name));
...
void Context::visitMemory(PNode* mem) {
  ...
  if (info->isAggr()) {
    ...
    for (int i = 5; i < mem->getChildNum(); i ++) {
      // AggrParentNode* parent = new AggrParentNode(path->abspath(SEP_AGGR, mem->getChild(i)->name));
      /* data & mask is unused */
      ...
      for (AggrParentNode* parent : info->aggrParent) {
        ...
        if (memPort->type == P_READER) { // data only
          AggrParentNode* portAggr = new AggrParentNode(path->abspath(SEP_MODULE, memPort->name + SEP_AGGR + "data" + suffix));
          ...
          AggrParentNode* maskAggr = new AggrParentNode(path->abspath(SEP_MODULE, memPort->name + SEP_AGGR + "mask" + suffix));
          AggrParentNode* dataAggr = new AggrParentNode(path->abspath(SEP_MODULE, memPort->name + SEP_AGGR + "data" + suffix));
          for (auto entry : parent->member) {
          ...
          AggrParentNode* wmaskAggr = new AggrParentNode(path->abspath(SEP_MODULE, memPort->name + SEP_AGGR + "wmask" + suffix));
          AggrParentNode* rdataAggr = new AggrParentNode(path->abspath(SEP_MODULE, memPort->name + SEP_AGGR + "rdata" + suffix));
          AggrParentNode* wdataAggr = new AggrParentNode(path->abspath(SEP_MODULE, memPort->name + SEP_AGGR + "wdata" + suffix));
          for (auto entry : parent->member) {
...
void Context::visitChirrtlMemPort(PNode* port) {
  ...
  std::string memName = path->abspath(SEP_MODULE, port->getExtra(0));
 ...
 if (port->type == P_READ && memoryMap[memName].first[0]->rlatency == 1) {
  ...
  for (AggrParentNode* memParent : memoryMap[memName].second) {
    ...
    AggrParentNode* parent = new AggrParentNode(path->abspath(SEP_MODULE, port->name) + suffix);
    for (auto entry : memParent->member) {
      std::string memberSuffix = replacePrefix(memName, "", entry.first->name).c_str();
      Node* memberNode = getSignal(path->abspath(SEP_MODULE, port->name) + memberSuffix);
...
Node* Context::allocCondNode(ASTExpTree* condExp, PNode* when) {
  Node* cond = allocNode(NODE_OTHERS, path->abspath(SEP_MODULE, "WHEN_COND_" + std::to_string(when->lineno)), when->lineno);
