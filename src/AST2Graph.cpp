...
static inline Node* allocNode(NodeType type = NODE_OTHERS, std::string name = "", int lineno = -1) {
  ...
  // Capture current hierarchy structure information
  if (!prefixTrace.empty()) {
    std::vector<std::string> hierarchyPath;
    std::vector<std::string> hierarchySeps;
    
    // Parse the top of current prefixTrace
    std::string currentPrefix = prefixTrace.top();
    if (!currentPrefix.empty()) {
      // Parse hierarchy path, separate module names and separators
      size_t pos = 0;
      std::string token;
      
      while (pos < currentPrefix.length()) {
        // Find next separator
        size_t sepPos = currentPrefix.find(globalConfig.sep_module, pos);
        if (sepPos == std::string::npos) sepPos = currentPrefix.find(globalConfig.sep_aggr, pos);
        
        if (sepPos == std::string::npos) {
          // Last part
          token = currentPrefix.substr(pos);
          if (!token.empty()) {
            hierarchyPath.push_back(token);
            hierarchySeps.push_back(""); // No separator for the last one
          }
          break;
        } else {
          // Found separator
          token = currentPrefix.substr(pos, sepPos - pos);
          if (!token.empty()) {
            hierarchyPath.push_back(token);
            
            // Determine separator type
            if (sepPos + globalConfig.sep_module.length() <= currentPrefix.length() &&
                currentPrefix.substr(sepPos, globalConfig.sep_module.length()) == globalConfig.sep_module) {
              hierarchySeps.push_back(globalConfig.sep_module);
              pos = sepPos + globalConfig.sep_module.length();
            } else if (sepPos + globalConfig.sep_aggr.length() <= currentPrefix.length() &&
                       currentPrefix.substr(sepPos, globalConfig.sep_aggr.length()) == globalConfig.sep_aggr) {
              hierarchySeps.push_back(globalConfig.sep_aggr);
              pos = sepPos + globalConfig.sep_aggr.length();
            } else {
              pos = sepPos + 1; // Default: skip one character
            }
          } else {
            pos = sepPos + 1;
          }
    ...
    node->hierarchySeps = hierarchySeps;
  }
