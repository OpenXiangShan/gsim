...
void addEdge(Node* src, Node* dst) {
  ...
  if (dst->type == NODE_MEMBER) { dst = dst->regNext; }
  if (src->type == NODE_MEMBER) { src = src->regNext; }
  dst->inEdge++;
...
graph* AST2Garph(PNode* root) {
  ...
  for (auto n : allSignals) {
    if (n.second->type == NODE_OTHERS && n.second->inEdge == 0) { g->constant.push_back(n.second); }
  }
