...
#define POTENTIAL_TYPE(n)                                                            \
  ((n->type == NODE_REG_DST) || (n->type == NODE_ACTIVE) || (n->type == NODE_OUT) || \
   (n->type == NODE_OTHERS) || (n->type) == NODE_MEMBER)
...
#define EXPR2_FUNC_TYPE \
  void (*)(mpz_t & dst, mpz_t & src1, mp_bitcnt_t bitcnt1, mpz_t & src2, mp_bitcnt_t bitcnt2)
...
static std::map<std::string, std::tuple<EXPR2_FUNC_TYPE, EXPR2_FUNC_TYPE>> expr2Map = {
    {"add", {us_add, us_add}},      {"sub", {us_sub, us_sub}},
    {"mul", {us_mul, us_mul}},      {"div", {us_div, us_div}},
    {"rem", {us_rem, us_rem}},      {"lt", {us_lt, us_lt}},
    {"leq", {us_leq, us_leq}},      {"gt", {us_gt, us_gt}},
    {"geq", {us_geq, us_geq}},      {"eq", {us_eq, us_eq}},
    {"neq", {us_neq, us_neq}},      {"dshl", {u_dshl, invalidExpr2}},
    {"dshr", {u_dshr, s_dshr}},     {"and", {u_and, invalidExpr2}},
    {"or", {u_ior, invalidExpr2}},  {"xor", {u_xor, invalidExpr2}},
...
static void cons_1expr1int(Node* node, int opIdx, int& prevIdx) {
  ...
  (op->sign ? std::get<1>(funcs) : std::get<0>(funcs))(val.back()->a, val.back()->a,
                                                       op->getChild(0)->width, n);
}
...
static void cons_2expr(Node* node, int opIdx, int& prevIdx) {
  ...
  (op->sign ? std::get<1>(funcs) : std::get<0>(funcs))(
      val[val.size() - 2]->a, val.back()->a, op->getChild(0)->width, val[val.size() - 2]->a,
      op->getChild(1)->width);
  deleteAndPop();
}
...
static void cons_1expr2int(Node* node, int opIdx, int& prevIdx) {
  ...
  if (op->sign)
    ...
    u_bits(val.back()->a, val.back()->a, op->getChild(0)->width, p_stoi(op->getExtra(0).c_str()),
