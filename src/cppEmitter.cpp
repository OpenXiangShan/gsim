...
void graph::genReset(SuperNode* super, bool isUIntReset, int indent) {
...
void graph::genResetAll() {
  ...
  for (SuperNode* super : uintReset) {
    ...
    genReset(super, true, 1);
  }
  emitBodyLock(0, "}\n");
  for (std::string str : resetFuncs) emitBodyLock(0, "%s", str.c_str());
}
