...
void insts_2expr(Node* node, int opIdx, int& prevIdx, bool nodeEnd) {
  ...
  if(result_mpz && operand_mpz) {
    ...
    if(valName.back().first > 0 && valName[valName.size() - 2].first > 0) {
      ...
      add_insts_4expr(node, FUNC_NAME(funcSign, op->name) + (op->getChild(0)->sign ? "_si_l" : "_ui_l"), dstName, "0x" + valName.back().second, std::to_string(op->getChild(0)->width), valName[valName.size() - 2].second, std::to_string(op->getChild(1)->width));
    } else {
    ...
    if(left_mpz) {
      ...
      add_insts_4expr(node, FUNC_SUI_L(funcSign, op->getChild(0)->sign, op->name), dstName, valName2Str(valName.back()), std::to_string(ABS(valName.back().first)), valName2Str(valName[valName.size()-2]), std::to_string(ABS(valName[valName.size()-2].first)));
    } else {
      node->insts.push_back(MPZ_SET(op->getChild(0)->sign) + "(" + dstName + ", " + valName2Str(valName.back()) + ")");
      add_insts_3expr(node, FUNC_SUI_R(funcSign, op->getChild(1)->sign, op->name), dstName, dstName, valName2Str(valName[valName.size()-2]), std::to_string(ABS(valName[valName.size()-2].first)));
    }
  } else if(!result_mpz && operand_mpz) {
...
void insts_1expr(Node* node, int opIdx, int& prevIdx, bool nodeEnd) {
  ...
  if(result_mpz) {
    ...
    if(valName.back().first > 64)
      ...
      add_insts_2expr(node, FUNC_NAME(op->sign, op->name) + "_ui", dstName, valName2Str(valName.back()), std::to_string(op->getChild(0)->width));
  } else if(!result_mpz && !operand_mpz) {
    if(opMap.find(op->name) != opMap.end())
...
void insts_mux(Node* node, int opIdx, int& prevIdx, bool nodeEnd) {
  ...
  if(node[opIdx].width > 64) {
    ...
    if(valName.back().first > 0) {
      ...
      if(valName.back().second.length() <= 16) cond_true = "mpz_set_ui(" + dstName + ", " + valName2Str(valName.back()) + ")";
    ...
    if(valName.back().first > 0) {
      ...
      if(valName.back().second.length() <= 16) cond_false = "mpz_set_ui(" + dstName + ", " + valName2Str(valName.back()) + ")";
...
void insts_printf(Node* node, int opIdx, int& prevIdx) {
  ...
  for(int i = 0; i < node->ops[opIdx]->getChild(2)->getChildNum(); i++) {
    inst += ", " + (valName.back().first > 0 ? valEqualsZero(valName.back().first, valName.back().second) : "0x" + valName.back().second);
    valName.pop_back();
  }
...
void insts_assert(Node* node, int opIdx, int& prevIdx) {
  if(!topValid) setPrev(node, prevIdx);
  std::string pred = valName.back().first > 0 ? valEqualsZero(valName.back().first, valName.back().second) : "0x" + valName.back().second;
  // std::string pred = valName.back().first ? (std::string("mpz_sgn(") + valName.back().second + ")") : "0x" + valName.back().second;
  valName.pop_back();
  std::string en = valName.back().first > 0 ? valEqualsZero(valName.back().first, valName.back().second) : "0x" + valName.back().second;
